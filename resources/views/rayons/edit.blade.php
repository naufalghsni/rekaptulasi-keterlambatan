...
<form method="post" action="{{ route('rayons.update', $rayon['id']) }}">
    @method('patch')
    @csrf
    <div class="mb-6">
      <label for="rayon" class="block text-gray-700 text-sm font-bold mb-2">Rayon:</label>
      <input type="text" id="rayon" name="rayon" class="w-full border rounded-md p-2" value="{{ old('rayon', $rayon->rayon) }}" required autofocus>
